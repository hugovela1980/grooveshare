...
The browser microphone adapter requests music-oriented capture by default (`echoCancellation: false`, `noiseSuppression: false`, `autoGainControl: false`) while keeping all browser APIs and constraint behavior inside `frontend-browser`. Checkpoint 5C also requests a single captured channel where the browser/audio route supports it (`channelCount: { ideal: 1 }`). This keeps ordinary mono microphone/interface input mono through the recording path so playback can be centered naturally across both output channels rather than presenting an input-1-only signal in one headphone channel. Because the constraint is an ideal rather than a guarantee, hardware validation remains part of 5C closure. Platform latency that remains after those constraints is treated separately from the musical timeline.
...
Checkpoint 5C also hardens Android wired-headphone playback against USB-C DACs that aggressively auto-mute sparse Web Audio output. Real-device diagnostics showed a JXMOX USB-C-to-3.5 mm adapter could remain inaudible for several beats even while an analyser immediately before `AudioContext.destination` showed correctly timed project audio. Increasing Web Audio scheduling lead to multiple seconds did not help. A zero-mean 20 kHz keepalive at -54 dBFS made starts reliable when active at least 250 ms before playback; the tested route remained reliably awake for about 500 ms after activity stopped, so GrooveShare uses a conservative 400 ms recent-activity window for rapid restarts. The compatibility decorator lives in `frontend-browser`, starts the keepalive only for playback/recording/review activity, stops it when playback becomes idle, and never changes project position, alignment compensation, or the shared 30 ms Web Audio scheduling lead. The measured constants are compatibility defaults, not musical timing values, and physical-device validation remains required because USB DAC behavior varies.

The Project Player also preserves the existing Go-to-bar input value when the mix reloads (including after a kept take is added), so recording/retry work does not unnecessarily reset the collaborator's current navigation target. Stopping a recording pass returns the project transport to that take's authoritative recording-start position, and stopped-take audition cleanup returns to the same position so Retry begins from the same musical location. The ordinary Project Player Stop action retains its normal reset-to-project-start behavior.

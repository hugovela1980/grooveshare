  ...
  const appElement = {
    ...
    querySelector<T>(selector: string): T | null {
      const elements = new Map<string, unknown>([
        ...
        ["#mobile-nav-auth-button", authButton],
  ...
  const appElement = {
    ...
    querySelector<T>(selector: string): T | null {
      const elements = new Map<string, unknown>([
        ...
        ["#mobile-nav-logout-button", logoutButton],
      ]);
...
tester.describe("mobile Guest invitation integration", () => {
  tester.it("opens an invitation without auth and removes the raw token from the normal project URL", async () => {
    ...
    try {
      ...
      tester.expect(appElement.innerHTML.includes("mobile-nav-auth-button")).toBe(true);
      tester.expect(appElement.innerHTML.includes("player-back-button")).toBe(false);
      tester.expect(appElement.innerHTML.includes(">Log In</span>")).toBe(true);
  ...
  tester.it("shows the active invitation on Projects only after the user is authenticated", async () => {
    ...
    try {
      ...
      tester.expect(projectListElement.innerHTML.includes("Invited Guest")).toBe(true);
      tester.expect(
        projectListElement.innerHTML.includes("Collaboration invitation available"),
      ).toBe(true);
      tester.expect(
